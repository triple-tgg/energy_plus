...
const previousMonthKey = (timeZone: string): string => {
    const now = new Date();
    const firstOfCurrentMonth = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
    return formatMonthKey(new Date(firstOfCurrentMonth.getTime() - 24 * 60 * 60 * 1000), timeZone);
};
...
export class AggregationService {
    ...
    async aggregatePreviousMonth(): Promise<void> {
        await this.aggregateMonthly(previousMonthKey(aggregationConfig.timezone));
    }
