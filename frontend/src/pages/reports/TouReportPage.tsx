...
const TouReportPage: React.FC = () => {
    ...
    const handleExport = async (format: ReportExportFormat) => {
        ...
        try {
            ...
            exportReport(exportRows, `tou_report_${today}`, 'TOU Report', format);
        } catch (err) {
        ...
        <div>
            ...
            <div style={{ background: C.bar, color: '#fff', display: 'flex', alignItems: 'stretch', borderBottom: `2px solid ${C.accent}`, marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px' }}>
                    ...
                    <div>
                        <div style={{ fontFamily: MONO, fontWeight: 700, fontSize: 13, letterSpacing: 2 }}>REPORTS // TOU</div>
                        <div style={{ fontSize: 10, color: C.barSub, letterSpacing: 0.5 }}>{t('รายงานการใช้พลังงานและการคิดค่าไฟฟ้าตามช่วงเวลา TOU (Time of Use)', 'Summary report of energy consumption and billing based on Time of Use (TOU)')}</div>
                    </div>
                </div>
            </div>
            <FilterBar
            ...
            />
            <DataTable title={t('รายงานการใช้พลังงานตามช่วงเวลา TOU', 'TOU Energy Consumption Report')} columns={columns} data={data} total={total} page={page} limit={limit} loading={loading} onPageChange={setPage} onLimitChange={(l) => { setLimit(l); setPage(1); }} onSearch={(search) => { setPage(1); setCurrentFilters(prev => ({ ...prev, search })); }} />
        </div>
    );
};
